...
func TestRun(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			opt:  options.WithIterativeConvergance(0.01, options.Batch, options.None, 10),
			want: []float64{0.874, 1.1},
		},
		{
			name: "batch automatic alpha=0.01 t=0.01",
			opt:  options.WithAutomaticConvergance(0.01, options.Batch, options.None, 0.01),
			want: []float64{0.872, 1.101},
		},
		{
			name: "stochastic iterative alpha=0.01 i=10000",
			opt:  options.WithIterativeConvergance(0.01, options.Stochastic, options.None, 10000),
			want: []float64{1, 1},
		},
		{
			name: "stochastic automatic alpha=0.01 t=0.01",
			opt:  options.WithAutomaticConvergance(0.01, options.Stochastic, options.None, 0.01),
...
func TestRun_TooLargeLearningRate(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			opt:  options.WithIterativeConvergance(0.01, options.Batch, options.None, 100),
		},
		{
			name: "batch automatic alpha=0.6 t=0.001",
			opt:  options.WithAutomaticConvergance(0.6, options.Batch, options.None, 0.001),
		},
		{
			name: "stochastic iterative alpha=0.2 i=100000",
			opt:  options.WithIterativeConvergance(0.2, options.Stochastic, options.None, 100000),
		},
		{
			name: "stochastic automatic alpha=0.1 t=0.01",
			opt:  options.WithAutomaticConvergance(0.1, options.Stochastic, options.None, 0.01),
		},
	}
	ctx := context.Background()
	for _, tt := range tests {
