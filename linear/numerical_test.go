...
func TestRun_WithGradientDescent(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			path:    "n=1_m=97.txt",
			options: options.WithAutomaticConvergance(0.0001, options.Batch, 0.0000001),
			want:    expected{r2: 0.702, coeffs: []float64{-3.858, 1.189}},
		},
		{
			name:    "stochastic automatic n=1 m=97 alpha=0.0001 t=0.0000000001",
			path:    "n=1_m=97.txt",
			options: options.WithAutomaticConvergance(0.0001, options.Stochastic, 0.0000000001),
